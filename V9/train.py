...
def set_seed(seed=42):
    ...
    if torch.cuda.is_available():
        ...
        torch.backends.cudnn.benchmark = True  # 训练加速
...
def setup_distributed():
    import os
    os.environ['MASTER_ADDR'] = '127.0.0.1'  # Windows下必须用127.0.0.1
...
def main():
    ...
    num_workers = 0  # Windows下分布式+多进程容易报错，先设为0
    train_sampler = DistributedSampler(train_ds, num_replicas=1, rank=0, shuffle=True)
    val_sampler = DistributedSampler(val_ds, num_replicas=1, rank=0, shuffle=False)
    test_sampler = DistributedSampler(test_ds, num_replicas=1, rank=0, shuffle=False)
    ...
    test_loader = DataLoader(test_ds, batch_size=batch_size, sampler=test_sampler, num_workers=num_workers, pin_memory=True)
    ...
    if mode == 'train':
        for epoch in range(1, epochs + 1):
            ...
            if is_main_process and val_loss < best_val_loss:
                ...
                torch.save(model.module.state_dict(), os.path.join(ckpt_dir, 'best_model.pth'))
        ...
        if is_main_process and latest_model and os.path.exists(latest_model):
