...
class SlidingWindowDataset(Dataset):
    ...
    def __init__(self,
        ...
        for b_idx, block in enumerate(self.blocks):
            ...
            for seg_start, seg_end in zip(seg_starts, seg_ends):
                seg_label = labels[seg_start]  # 区间标签
    ...
    def __getitem__(self, idx):
        ...
        data_array = window_df[self.feature_cols].values[::self.sampling_rate]        # 确保数据是数值型的
        # 先转换为DataFrame来处理类型问题
        import pandas as pd
...
def create_dataset_from_config(cfg_path: str, phase: str = "encode", need_indices: list[int] = None, dynamic_need: bool = False) -> SlidingWindowDataset:
    ...
    if label_col in data.columns:
        data[label_col] = pd.to_numeric(data[label_col], errors='coerce').fillna(0)
    # 自动推断need_indices
