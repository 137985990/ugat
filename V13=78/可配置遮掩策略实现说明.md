...
**好消息**：V13版本**已经支持**暂停后继续训练！训练脚本中实现了完整的检查点保存和恢复机制。

#### 12.1.1 **现有功能特性**

1. **自动检查点保存**
   - 每当验证损失改善时自动保存最佳模型
   - 周期性保存检查点（默认每10个epoch）
   - 保存完整训练状态，包括优化器、调度器等

2. **智能恢复机制**
   - 自动检测现有检查点文件
   - 完整恢复训练状态，包括epoch数、最佳损失等
   - 支持混合精度训练状态恢复

3. **多重备份策略**
   - 完整检查点文件：`best_model.pth`
   - 仅权重文件：`best_model_weights_only.pth`
   - 周期性检查点：`checkpoint_epoch_N.pth`
...
save_interval: 10               # 周期性保存间隔（epoch）
...
checkpoint = {
    ...
    'scaler_state_dict': {...},            # 混合精度缩放器状态（如果使用）
    'best_val_loss': 0.0234,              # 最佳验证损失
    'config': {...},                       # 完整配置信息
    ...
    'train_loss': 0.0267,
    'train_recon': 0.0198,
    'train_input_acc': 0.8234,            # 输入数据准确率
    'train_reconstructed_acc': 0.8756,     # 重建数据准确率
    'val_input_acc': 0.8123,
    'val_reconstructed_acc': 0.8567,
    'accuracy_improvement': 0.0522,        # 准确率提升
    'val_accuracy_improvement': 0.0444
}
```
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                if 'train_input_acc' in checkpoint:
                    logger.info(f"上次训练表现:")
                    logger.info(f"  训练准确率提升: {checkpoint.get('accuracy_improvement', 0):.4f}")
                    logger.info(f"  验证准确率提升: {checkpoint.get('val_accuracy_improvement', 0):.4f}")
...
2. **周期性保存**（每N个epoch）：
   ...
   if epoch % save_interval == 0:
       periodic_checkpoint = {...}  # 包含当前状态
       torch.save(periodic_checkpoint, f'Checkpoints/checkpoint_epoch_{epoch}.pth')
   ```
...
```bash
# 第一次训练（训练了50个epoch后手动停止）
python train.py --config config.yaml
# 输出：训练到第50轮，保存检查点

# 恢复训练（设置resume_training: true）
python train.py --config config.yaml
# 输出：恢复训练从第51轮开始，最佳验证损失: 0.0234
```
...
ls Checkpoints/
# 输出：
# best_model.pth (最佳模型，epoch 45)
# checkpoint_epoch_40.pth
# checkpoint_epoch_50.pth (最新周期保存)
...
```yaml
# 修改config.yaml中的某些超参数
learning_rate: 1e-5        # 从1e-4调整为1e-5
cls_improvement_weight: 2.5 # 从2.0调整为2.5
resume_training: true       # 启用恢复
...
```python
# 加载特定epoch的检查点
def load_specific_checkpoint(epoch_num):
    checkpoint_path = f'Checkpoints/checkpoint_epoch_{epoch_num}.pth'
...
def list_available_checkpoints():
    ckpt_files = glob.glob('Checkpoints/checkpoint_epoch_*.pth')
    epochs = [int(f.split('_')[-1].split('.')[0]) for f in ckpt_files]
...
def inspect_checkpoint(checkpoint_path):
    ...
    print(f"Train Accuracy Improvement: {checkpoint.get('accuracy_improvement', 'Unknown')}")
    print(f"Config: {checkpoint.get('config', {}).get('training_strategy', 'Unknown')}")
    
    return checkpoint

# 使用示例
inspect_checkpoint('Checkpoints/best_model.pth')
```
...
- ✅ 模型架构参数不能改变（如`hidden_channels`、`num_classes`等）
- ✅ 训练超参数可以调整（如`learning_rate`、`batch_size`等）
- ✅ 损失函数权重可以动态调整
- ❌ 数据集结构不建议大幅改变
...
save_interval: 10              # 平衡存储空间和恢复粒度
cleanup_old_checkpoints: true  # 自动清理旧检查点（保留最近3个）
```
...
V13版本**完全支持**暂停后继续训练，具备以下优势：

1. **✅ 零配置恢复**：只需设置`resume_training: true`
2. **✅ 完整状态保存**：包括优化器、调度器、奖励机制状态
3. **✅ 智能检测机制**：自动识别和加载最佳检查点
4. **✅ 多重备份策略**：确保训练状态不丢失
5. **✅ 灵活参数调整**：恢复时可调整部分超参数
6. **✅ 详细日志记录**：完整的恢复过程追踪

**使用方法**：在配置文件中设置`resume_training: true`，然后正常运行训练脚本即可。系统会自动检测并恢复之前的训练状态。
