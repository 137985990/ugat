...
def create_dataset_from_config(cfg_path: str, phase: str = "encode", need_indices: list[int] = None, dynamic_need: bool = False) -> SlidingWindowDataset:
    ...
    return ds
    def update_need(self, predictions: torch.Tensor, indices: list[int] = None):
        """
        批量更新数据集中所有 need 通道（untrusted modalities），用模型预测结果覆盖。
        Args:
            predictions: Tensor of shape [num_windows, num_need, window_size]，每个窗口的need通道预测值。
            indices: 对应 self.indices 的窗口索引（可选，默认顺序对应）。
        """
        if not self.need_indices or predictions is None:
            return
        if indices is None:
            indices = range(len(self.indices))
        assert len(indices) == predictions.shape[0], "预测数量与窗口数量不符"
        # 逐窗口写回DataFrame
        for i, idx in enumerate(indices):
            b_idx, start = self.indices[idx]
            block = self.blocks[b_idx]
            window_df = block.iloc[start:start + self.window_size]
            for n, need_idx in enumerate(self.need_indices):
                col = self.feature_cols[need_idx]
                # 用预测值覆盖window_df[col]
                pred_vals = predictions[i, n, :].detach().cpu().numpy()
                # 写回原DataFrame
                block.iloc[start:start + self.window_size, block.columns.get_loc(col)] = pred_vals
        # 更新self.data（重新聚合blocks）
        self.data = pd.concat(self.blocks, axis=0).reset_index(drop=True)
        # blocks已是引用，无需重建
        # 注意：如需重新分组/索引，需重建blocks和indices
